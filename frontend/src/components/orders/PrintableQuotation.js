...
const QUOTATION_A5_ROW_HEIGHT_MM = 6.2;

const getQuotationPrintCss = (pageSize = 'A4') => {
  const dims = getBillDimensions(pageSize);
  const compact = pageSize === 'A5';
  const rowHeight = compact ? QUOTATION_A5_ROW_HEIGHT_MM : QUOTATION_ROW_HEIGHT_MM;
  ...
  @media print {
    ...
    .quotation-bill-body {
      padding-bottom: ${compact ? '22px' : '34px'} !important;
      overflow: hidden !important;
    }
    .quotation-customer-block {
      padding: ${compact ? '3px 8px' : '6px 10px'} !important;
      overflow: hidden !important;
    }
    ...
    .quotation-table-section {
      padding: 0 ${compact ? '8px' : '10px'} !important;
      overflow: hidden !important;
    }
    ...
    .quotation-bill-a4 .quotation-bill-table { font-size: ${compact ? '8px' : '10.5px'}; }
    .quotation-bill-a4 .quotation-bill-table th { font-size: ${compact ? '8px' : '10.5px'} !important; }
    .quotation-bill-a4 .quotation-bill-table td { font-size: ${compact ? '8px' : '11px'} !important; }
    .quotation-bill-a4 .quotation-bill-table th,
    .quotation-bill-a4 .quotation-bill-table td {
      padding: ${compact ? '1px 2px' : '2px 3px'} !important;
    }
    .quotation-bill-a4 .quotation-bill-table tbody tr.quotation-bill-row {
      height: ${rowHeight}mm !important;
      max-height: ${rowHeight}mm !important;
    }
    ...
    .watermark-quotation {
      ...
      opacity: 1 !important;
      font-size: ${compact ? '40px' : '64px'} !important;
...
const PrintableQuotation = ({ order, customer }) => {
  ...
  const rowHeightMm = isA5 ? QUOTATION_A5_ROW_HEIGHT_MM : QUOTATION_ROW_HEIGHT_MM;
  const rowHeight = `${rowHeightMm}mm`;
  const thStyle = {
    border: '1px solid #0047AB',
    padding: isA5 ? '1px 2px' : '3px 3px',
    textAlign: 'center',
    fontSize: isA5 ? '8px' : '10.5px',
    lineHeight: 1.2,
  };
  const tdStyle = {
  ...
  return (
    <div className="w-full flex flex-col items-center py-4 bg-gray-50 dark:bg-gray-900">
      ...
      <div
        ...
        style={{
          ...getBillContainerStyle(pageSize),
          maxWidth: '100%',
          height: getBillDimensions(pageSize).height,
          padding: 0,
      ...
      >
        ...
        <div className="quotation-bill-body" style={{ position: 'relative', zIndex: 1, paddingBottom: isA5 ? '22px' : '34px' }}>
        {/* Blue Header with Logo */}
        <div className="flex items-stretch" style={{ height: isA5 ? '38px' : '68px', flexShrink: 0 }}>
          <div className="flex items-center justify-center" style={{ 
            width: '35%', 
            backgroundColor: '#0047AB',
            padding: isA5 ? '3px' : '8px'
          }}>
            <img 
              src={systemLogo} 
              alt="Wahid Afghan Logo" 
              style={{ 
                width: isA5 ? '30px' : '54px', 
                height: isA5 ? '30px' : '54px', 
                borderRadius: '50%', 
                objectFit: 'cover',
                border: isA5 ? '2px solid white' : '3px solid white',
        ...
        {/* Quotation Label */}
        <div style={{ 
          padding: isA5 ? '2px 10px' : '5px 14px', 
          backgroundColor: '#0047AB', 
          color: 'white',
          textAlign: 'center',
        ...
        <div className="quotation-customer-block" style={{ padding: isA5 ? '3px 8px' : '6px 14px', backgroundColor: '#f8f9fa', flexShrink: 0 }} dir="rtl">
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '2px' }}>
            <div style={{ fontSize: isA5 ? '8px' : '10px' }}>
          ...
          {(order.manual_serial_no || '').trim() !== '' && (
            <div style={{ fontSize: isA5 ? '8px' : '10px', marginBottom: '2px', borderBottom: '1px dotted #999', paddingBottom: '1px' }}>
          ...
          <div style={{ fontSize: isA5 ? '8px' : '10px', borderBottom: '1px dotted #999', paddingBottom: '1px' }}>
          ...
          {order.notes && String(order.notes).trim() ? (
            <div style={{ fontSize: isA5 ? '8px' : '10px', marginTop: '2px', paddingTop: '2px', borderTop: '1px dotted #ccc' }} dir="rtl">
        ...
        <div className="quotation-table-section" style={{ padding: isA5 ? '0 8px' : '0 12px' }}>
          <table className="quotation-bill-table" style={{ width: '100%', borderCollapse: 'collapse', marginTop: isA5 ? '2px' : '4px', tableLayout: 'fixed' }}>
            <thead>
        ...
        {/* Totals Section */}
        <div style={{ padding: isA5 ? '4px 8px' : '8px 12px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexShrink: 0 }} dir="rtl">
          <div style={{ 
            border: isA5 ? '2px solid #0047AB' : '3px solid #0047AB',
            padding: isA5 ? '3px 12px' : '6px 20px',
            clipPath: 'polygon(0 0, 100% 0, 90% 50%, 100% 100%, 0 100%)',
            minWidth: isA5 ? '110px' : '140px'
          }}>
            <div style={{ fontSize: isA5 ? '10px' : '12px', fontWeight: 'bold', textAlign: 'center' }}>
              مجموع پول:
            </div>
            <div style={{ fontSize: isA5 ? '12px' : '14px', fontWeight: 'bold', textAlign: 'center', marginTop: '2px' }}>
              {grandTotal.toFixed(0)}
            </div>
          </div>

          <div style={{ flex: 1, paddingRight: isA5 ? '12px' : '20px' }}>
            <div style={{ fontSize: isA5 ? '8px' : '10px', marginBottom: isA5 ? '2px' : '4px', borderBottom: '1px dotted #999', paddingBottom: '2px' }}>
            ...
            <div style={{ textAlign: 'center', marginTop: isA5 ? '4px' : '10px', fontSize: isA5 ? '8px' : '10px' }}>
               مهر و امضاء
              <div style={{ borderTop: '1px solid #000', marginTop: isA5 ? '6px' : '14px', width: isA5 ? '90px' : '120px', marginLeft: 'auto', marginRight: 'auto' }}></div>
            </div>
          </div>
        </div>
        </div>

        {/* Yellow Footer — pinned to bottom of the page */}
        <div className="quotation-bill-footer" style={{ 
          backgroundColor: '#FFD700', 
          padding: isA5 ? '3px 8px' : '6px 12px',
          display: 'flex',
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: isA5 ? '6px' : '10px', fontSize: isA5 ? '7.5px' : '10px' }}>
          ...
          <div style={{
            backgroundColor: '#0047AB',
            color: 'white',
            padding: isA5 ? '3px 12px 3px 8px' : '5px 20px 5px 12px',
            fontSize: isA5 ? '7px' : '9px',
            clipPath: 'polygon(15% 0, 100% 0, 100% 100%, 15% 100%, 0 50%)',
            textAlign: 'right',
            maxWidth: isA5 ? '260px' : '360px'
          }} dir="rtl">
      ...
      <style>{getQuotationPrintCss(pageSize)}</style>
    </div>
  );
};
